...
const DevDesign = () => {
  ...
  return (
    <>
      <Div type="flex" direction="vertical" className={cx("")}>
        {showMenu ? (
          <Div className="width-px-200 height-vh-full of-y-auto bg-yellow scroll-type-one">
            {DESIGN_ITEMS?.map((item, idx) => (
              <Div
                key={idx}
                className={cx(
                  "p-y-8 p-x-16 text-center m-b-8 text-black mouse-hand bg-silver"
                )}
                onClick={() => {
                  setShowMenu(false);
          ...
          <Div
            ...
            direction="vertical"
            hAlign="center"
            className={cx("width-per-100 height-vh-full of-y-auto bg-green")}
          >
            <Div
              type="flex"
              hAlign="center"
              vAlign="center"
              className="pos-abs bg-silver br-rad-per-50 width-px-30 height-px-30 mouse-hand"
            ...
            <Div
              type="flex"
              hAlign="center"
              className={cx("width-per-100 flex--wrap")}
            >
