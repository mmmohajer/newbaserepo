...
This project integrates with multiple third-party services (Google Cloud, OpenAI, AWS, Azure, SendGrid, Stripe, etc.).  
...
- [6. Stripe (Payments & Billing)](#6-stripe-payments--billing)
  - [6.1 Create an Account](#61-create-an-account)
  - [6.2 Get Your API Keys](#62-get-your-api-keys)
  - [6.3 Configure Webhooks](#64-configure-webhooks)
...
- Verify your email and complete the business setup process.

### 6.2 Get Your API Keys

1. Go to the **Stripe Dashboard** → **Developers** → **API Keys**.
2. Copy your **Publishable key** (for the frontend) and **Secret key** (for the backend).

💡 **Tip:**

- Keep test and production keys separate.
- For production deployment, inject these securely via your CI/CD pipeline, Docker secrets, or environment variables — never commit them to source control.

### 6.3 Configure Webhooks

1. Go to the **Stripe Dashboard** → **Developers** → **Webhooks**.
2. Click **+ Add endpoint**, and set your webhook URL (e.g. `https://yourapp.com/api/stripe/webhook/`).
3. Under **Select events to send**, add the following:
...
4. After saving, copy the **Signing secret** (`whsec_...`) — this is your `STRIPE_WEBHOOK_SECRET_KEY` value.
